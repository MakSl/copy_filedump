...
def format_header(buffer, current_block, block_size):
    ...
    print(
        ...
        % (page_offset, values[4], values[4])
    )
    print(
        f"Block: Size {values[7] & (0xFF00):4d}  Version {values[7] & (0x00FF):4}            Upper    {values[5]:4} (0x{values[5]:04x})"
    )
    print(
        "LSN:  logid %6d recoff 0x%08x      Special  %4u (0x%04hx)"
        % (values[0], values[1], values[6], values[6])
    )
    print(f"Items: {max_offset:4d}                      Free Space: {values[5] - values[4]:4}")
    print(
        f"Checksum: 0x{values[2]:04x}  Prune XID: 0x{values[8]:08x}  Flags: 0x{values[3]:04x} ({flag_string})"
    )
    print(f"Length (including item array): {header_bytes}\n")
...
def Format_item_block(buffer):
    ...
    if max_offset == 0:
        ...
        for x in range(1, max_offset + 1):
            ...
            print(
                f"Item {x:3d} -- Length: {item_size:4d}  Offset: {item_offset:4d} (0x{item_offset:04x})  Flags: {text_flags}"
            )
            ...
            print(
                f"  XMIN: {2 if (values2[7] & 0x0300) == 0x0300 else values2[0]} "
                f"XMAX: {values2[1]} "
                f"CID|XVAC: {values2[2]}"
            )

            print(
                f"  Block Id: {(values2[3] << 16) | values2[4]}  "
            ...
            values2 = struct.unpack(
                "B" * bitmap_length, buffer[item_offset + 23 : item_offset + 23 + bitmap_length * 1]
            )
